...
public class LcdPcf8574 {
    ...
    public LcdPcf8574(Config config) {
        this.config = config;
    }
    ...
    public interface Config {
        void write(byte[] data, int command) throws IOException;
    }
    ...
    private void write2Wire(byte halfByte, int mode, boolean enable) throws IOException {
        ...
        config.write(new byte[]{i2cData}, 1);
    }

    private void delayMicroseconds(int microseconds) {
        try {
            Thread.sleep(Math.max(1, Math.round(0.001f * microseconds)));
